...
const { packageName } = require("./bib.cjs");

test("platform packages and unsupported platforms", () => {
  for (const platform of ["linux", "darwin"]) {
    for (const arch of ["x64", "arm64"]) {
      assert.equal(packageName(platform, arch), `@brokkai/issue-bot-${platform}-${arch}`);
    }
  }
...
function fixture(t, source) {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "bib-npm-"));
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  const launcher = path.join(directory, "bib.cjs");
  fs.copyFileSync(path.join(__dirname, "bib.cjs"), launcher);
  const binary = path.join(directory, "node_modules", packageName(), "bin", "bib");
