...
fail() {
    printf 'brokk-issue-bot installer: %s\n' "$*" >&2
    exit 1
}
...
main() {
    ...
    releases=https://github.com/BrokkAi/issue-bot/releases
    version=${1:-latest}
    ...
    [ ! -d "$install_dir/bib" ] || fail 'installation target is a directory'
    temporary=$(mktemp -d)
    staged=
    trap 'rm -rf "$temporary"; if [ -n "$staged" ]; then rm -f "$staged"; fi' 0
    trap 'exit 1' HUP INT TERM

    asset=brokk-issue-bot-$version-$os-$arch.tar.gz
    printf 'Downloading brokk-issue-bot %s for %s/%s...\n' "$version" "$os" "$arch"
    ...
    mkdir -p "$install_dir"
    staged=$(mktemp "$install_dir/.brokk-issue-bot.XXXXXX")
    # Extract only the binary to a fresh file; never follow archive paths or links.
    tar -xzOf "$temporary/$asset" bib > "$staged" || fail 'could not extract bib'
    [ -s "$staged" ] || fail 'archive contains an empty binary'
    chmod 755 "$staged"
    mv -f "$staged" "$install_dir/bib"
    staged=
    printf 'Installed brokk-issue-bot %s to %s/bib\n' "$version" "$install_dir"
    case ":${PATH:-}:" in
        *":$install_dir:"*) ;;
        *) printf 'Add %s to your PATH, then run bib --help.\n' "$install_dir" ;;
    esac
}
