...
type dashboard struct {
	...
	streamKey                                      string
	streamOpen                                     bool
	completed, attempts, sessions, tools, failures int
...
func newDashboard(cfg bot.Config) *dashboard {
	...
	return &dashboard{cfg: cfg, started: now, phaseStarted: now, lastActivity: now,
		progress: bot.Progress{Phase: "starting", Task: "Loading saved issue progress"}}
}
...
func (d *dashboard) update(p bot.Progress) {
	...
	// Keep a issue open while new results arrive; the overview follows new
	// issues only when the user has not browsed away from the newest one.
	if (d.detail || d.selected > 0) && len(d.progress.Issues) > d.selected {
		id := d.progress.Issues[len(d.progress.Issues)-1-d.selected].ID
		for i, f := range p.Issues {
			if f.ID == id {
				d.selected = len(p.Issues) - 1 - i
				break
			}
		}
	}
	d.progress = p
	d.selected = max(0, min(d.selected, len(p.Issues)-1))
}
...
func (d *dashboard) key(key string) bool {
	...
	case "enter":
		if len(d.progress.Issues) > 0 && d.view != 2 {
	...
	case "j", "down", "pgdown":
		if d.view == 2 {
			...
			d.selected = max(0, min(len(d.progress.Issues)-1, d.selected+1))
		}
	case "k", "up", "pgup":
		if d.view == 2 {
			d.scroll++
	...
	case "G":
		if d.view == 2 {
			...
			d.selected = max(0, len(d.progress.Issues)-1)
		}
	}
	return false
}
...
func (d *dashboard) render(width, height int, now time.Time, color bool) string {
	...
	add("ISSUE BOT  "+d.cfg.GitHubRepo(), "1;36")
	if d.detail && len(p.Issues) > 0 {
		add(fmt.Sprintf("ISSUE DETAIL · %d/%d", d.selected+1, len(p.Issues)), "2")
		f := p.Issues[len(p.Issues)-1-d.selected]
	...
	c := p.Counts
	add(fmt.Sprintf("Saved: %d issues · %d PRs · %d existing PRs", c.Total, c.Submitted, c.ExistingPR), "1")
	if height >= 18 {
		add(fmt.Sprintf("%d pending · %d blocked · %d skipped", c.Pending, c.Blocked, c.Skipped), "2")
	}
	run := fmt.Sprintf("Run: %d completed · %d agents · %d tools", d.completed, d.sessions, d.tools)
	if width >= 65 {
		run = fmt.Sprintf("Run: %d completed · %d attempts · %d agents · %d tools · %d errors", d.completed, d.attempts, d.sessions, d.tools, d.failures)
	}
	add(run, "2")
	if height >= 18 {
		add("", "")
	}
	tabs := []string{"1 Overview", "2 Issues", "3 Activity"}
...
func (d *dashboard) issueRows(limit int) []string {
	if limit <= 0 {
		return nil
	}
	issues := d.progress.Issues
	if len(issues) == 0 {
		if d.progress.Phase == "waiting" || d.progress.Phase == "complete" {
			return []string{"No issues in saved history."}
		}
		return []string{"No issues yet."}
	}
	d.selected = max(0, min(d.selected, len(issues)-1))
	start := max(0, d.selected-limit+1)
	var rows []string
	for i := start; i < min(len(issues), start+limit); i++ {
		f := issues[len(issues)-1-i]
		prefix := "  "
		if i == d.selected {
			prefix = "› "
		}
...
func (d *dashboard) summary(err error) string {
	...
	if err == nil && !d.stopping {
		status = "complete"
		if d.progress.Phase == "paused" || d.progress.Phase == "blocked" {
			status = d.progress.Phase
		}
	}
	c := d.progress.Counts
	text := fmt.Sprintf("bib %s · %s · saved: %d issues, %d PRs, %d existing PRs\n", cleanText(d.cfg.GitHubRepo()), status, c.Total, c.Submitted, c.ExistingPR)
	for _, f := range d.progress.Issues {
	...
	if d.progress.Failure != "" {
		text += "Failure: " + cleanText(d.progress.Failure) + "\n"
	}
